...
public class SwerveDriveTest
{
  ...
  /**
   * Generates a command to test the direction of the angle motor in a SwerveDrive.
   *
   * @param swerveDrive the SwerveDrive object to test
   * @param module the name of the module to test
   * @return a command that sets the angle motor to 0.1, prints the current position and absolute position,
   *         waits until the position is greater than 10 and the absolute position is greater than 10,
   *         and finally prints a success message or an error message if interrupted.
   */
  public static Command generateAngleMotorDirectionTest(SwerveDrive swerveDrive, String module) {
    return Commands.run(() -> 
      swerveDrive.getModuleMap().get(module).getAngleMotor().set(0.1))
      .alongWith(Commands.print("Angle Motor Position: " + 
        swerveDrive.getModuleMap().get(module).getAngleMotor().getPosition() + " Absolute Position: " + 
        swerveDrive.getModuleMap().get(module).getAbsolutePosition()))
      .until(() -> swerveDrive.getModuleMap().get(module).getAngleMotor().getPosition() > 10 &&
        swerveDrive.getModuleMap().get(module).getAbsolutePosition() > 10)
      .andThen(Commands.print("Angle Motor Direction Test Passed: Angle Motor Position: " + 
        swerveDrive.getModuleMap().get(module).getAngleMotor().getPosition() + " Absolute Position: " + 
        swerveDrive.getModuleMap().get(module).getAbsolutePosition()))
      .withTimeout(5)
      .finallyDo((interrupted) -> System.err.println(interrupted ? "Interrupted" : "Passed"));
  }

  /**
   * Generates a command to test the rotation of the angle motor in a SwerveDrive.
   *
   * @param swerveDrive the SwerveDrive object to test
   * @param module the name of the module to test
   * @return a command that sets the angle motor to 45 degrees, prints the current position and absolute position,
   *         waits until the position is 45 and the absolute position is 45,
   *         and finally prints a success message or an error message if interrupted.
   */
  public static Command generateAngleMotorRotateTest(SwerveDrive swerveDrive, String module) {
    return Commands.run(() -> 
      swerveDrive.getModuleMap().get(module).getAngleMotor().setReference(45, 0))
      .alongWith(Commands.print("Angle Motor Position: " + 
        swerveDrive.getModuleMap().get(module).getAngleMotor().getPosition() + " Absolute Position: " + 
        swerveDrive.getModuleMap().get(module).getAbsolutePosition()))
      .until(() -> swerveDrive.getModuleMap().get(module).getAngleMotor().getPosition() == 45 &&
        swerveDrive.getModuleMap().get(module).getAbsolutePosition() == 45)
      .andThen(Commands.print("Angle Motor Direction Test Passed: Angle Motor Position: " + 
        swerveDrive.getModuleMap().get(module).getAngleMotor().getPosition() + " Absolute Position: " + 
        swerveDrive.getModuleMap().get(module).getAbsolutePosition()))
      .withTimeout(5)
  ...
  /**
   * Generates a command to test the direction of the drive motor in a SwerveDrive.
   *
   * @param swerveDrive the SwerveDrive object to test
   * @param module the name of the module to test
   * @return a command that sets the drive motor to 4 volts, prints the current position of the drive motor,
   *         waits until the position is greater than 0.10,
   *         and finally prints a success message or an error message if interrupted.
   */
    public static Command generateDriveMotorDirectionTest(SwerveDrive swerveDrive, String module) {
    return Commands.run(() -> 
      swerveDrive.getModuleMap().get(module).getDriveMotor().setVoltage(4))
      .alongWith(Commands.print("Drive Motor Position: " + 
        swerveDrive.getModuleMap().get(module).getDriveMotor().getPosition()))
      .until(() -> swerveDrive.getModuleMap().get(module).getDriveMotor().getPosition() > 0.10)
      .andThen(Commands.print("Drive Motor Direction Test Passed: Drive Motor Position: " + 
        swerveDrive.getModuleMap().get(module).getDriveMotor().getPosition()))
      .withTimeout(5)
      .finallyDo((interrupted) -> System.err.println(interrupted ? "Interrupted" : "Passed"));
  }
  ...
  /**
   * Generates a command to test the velocity of the drive motor in a SwerveDrive.
   *
   * @param swerveDrive the SwerveDrive object to test
   * @param module the name of the module to test
   * @return a command that sets the drive motor to a reference velocity of 1.0, prints the current velocity of the drive motor,
   *         waits until the velocity is equal to 1.0,
   *         and finally prints a success message or an error message if interrupted.
   */
  public static Command generateDriveMotorVelocityTest(SwerveDrive swerveDrive, String module) {
    return Commands.run(() -> 
      swerveDrive.getModuleMap().get(module).getDriveMotor().setReference(1.0, 0))
      .alongWith(Commands.print("Drive Motor Velocity: " + 
        swerveDrive.getModuleMap().get(module).getDriveMotor().getVelocity()))
      .until(() -> swerveDrive.getModuleMap().get(module).getDriveMotor().getVelocity() == 1.0)
      .andThen(Commands.print("Drive Motor Velocity Test Passed: Drive Motor Velocity: " + 
        swerveDrive.getModuleMap().get(module).getDriveMotor().getVelocity()))
      .withTimeout(5)
      .finallyDo((interrupted) -> System.err.println(interrupted ? "Interrupted" : "Passed"));
  }

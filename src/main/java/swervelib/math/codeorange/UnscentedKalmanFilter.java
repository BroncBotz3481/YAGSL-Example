...
/**
 * A Kalman filter combines predictions from a model and measurements to give an estimate of the true ystem state. This
    ...
    implements KalmanTypeFilter<States, Inputs, Outputs>
{
  ...
  private final BiFunction<Matrix<States, ?>, Matrix<?, N1>, Matrix<States, N1>>   m_meanFuncX;
  private final BiFunction<Matrix<Outputs, ?>, Matrix<?, N1>, Matrix<Outputs, N1>> m_meanFuncY;
  private final BiFunction<Matrix<States, N1>, Matrix<States, N1>, Matrix<States, N1>> m_residualFuncX;
  private final BiFunction<Matrix<Outputs, N1>, Matrix<Outputs, N1>, Matrix<Outputs, N1>> m_residualFuncY;
  private final BiFunction<Matrix<States, N1>, Matrix<States, N1>, Matrix<States, N1>>    m_addFuncX;
  ...
  /**
   * Constructs an unscented Kalman filter with custom mean, residual, and addition functions. Using custom functions
   * for arithmetic can be useful if you have angles in the state or measurements, because they allow you to correctly
   * account for the modular nature of angle arithmetic.
   *
   * @param states             A Nat representing the number of states.
   * @param outputs            A Nat representing the number of outputs.
   * @param f                  A vector-valued function of x and u that returns the derivative of the state vector.
   ...
   * @param stateStdDevs       Standard deviations of model states.
   * @param measurementStdDevs Standard deviations of measurements.
   * @param meanFuncX          A function that computes the mean of 2 * States + 1 state vectors using a given set of
   *                           weights.
   * @param meanFuncY          A function that computes the mean of 2 * States + 1 measurement vectors using a given set
   *                           of weights.
   * @param residualFuncX      A function that computes the residual of two state vectors (i.e. it subtracts them.)
   * @param residualFuncY      A function that computes the residual of two measurement vectors (i.e. it subtracts
   *                           them.)
   ...
   * @param h             A vector-valued function of x and u that returns the measurement vector.
   * @param R             Measurement noise covariance matrix (continuous-time).
   * @param meanFuncY     A function that computes the mean of 2 * States + 1 measurement vectors using a given set of
   *                      weights.
   * @param residualFuncY A function that computes the residual of two measurement vectors (i.e. it subtracts them.)
   * @param residualFuncX A function that computes the residual of two state vectors (i.e. it subtracts them.)
   * @param addFuncX      A function that adds two state vectors.
   */
  @SuppressWarnings({"ParameterName", "LocalVariableName"})
